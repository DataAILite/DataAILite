...
Public MustInherit Class MarketAnalysisBase
    ...
    Private Sub SelectListBoxValues(listBox As ListBox, ParamArray values() As String)
        If listBox Is Nothing OrElse values Is Nothing Then Exit Sub
        Dim selectedAny As Boolean = False
        listBox.ClearSelection()
        For Each valueText As String In values
            If valueText Is Nothing OrElse valueText.Trim() = "" Then Continue For
            Dim item As ListItem = listBox.Items.FindByValue(valueText.Trim())
            If item Is Nothing Then item = listBox.Items.FindByText(valueText.Trim())
            If item Is Nothing OrElse item.Selected Then Continue For
            item.Selected = True
            selectedAny = True
        Next

        If Not selectedAny Then SelectFieldsByHints(listBox, PreferredDimensionHints())
    End Sub
