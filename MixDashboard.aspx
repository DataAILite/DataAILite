<%@ Page Language="VB" AutoEventWireup="false" CodeFile="MixDashboard.aspx.vb" Inherits="MixDashboard" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Mixed Dashboard</title>
    ...
    <script type="text/javascript">
        ...
        function prepareDashboardExportImagesAndSubmit(button) {
            ...
            var originalFinishExport = finishExport;
            finishExport = function () {
                finishExport = originalFinishExport;
                var submitAfterVisuals = finishExport;
                finishExport = function () {
                    finishExport = submitAfterVisuals;
        ...
        function mixDashboardChartOptions(chrt) {
            if (chrt == "Map") {
                ...
                return {
                    ...
                    legend: { textStyle: { color: "black", fontSize: 8 } },
                    hAxis: { textStyle: { color: "black", fontSize: 8 } },
                    vAxis: { textStyle: { color: "black", fontSize: 8 } },
                    center: { lat: 37.06, lng: -95.68 },
                    showTooltip: true,
                    showInfoWindow: true,
                    useMapTypeControl: true,
                    icons: {
                        blue: { normal: url + "Map-Marker-Ball-Azure-icon.png", selected: url + "Map-Marker-Ball-Right-Azure-icon.png" },
                        green: { normal: url + "Map-Marker-Push-Pin-1-Chartreuse-icon.png", selected: url + "Map-Marker-Push-Pin-1-Right-Chartreuse-icon.png" },
                        pink: { normal: url + "Map-Marker-Ball-Pink-icon.png", selected: url + "Map-Marker-Ball-Right-Pink-icon.png" }
                    }
                };
            }
            if (chrt == "GeoChart") {
            ...
            if (chrt == "Gauge") {
                return { width: 400, height: 300, redFrom: 90, redTo: 100, yellowFrom: 75, yellowTo: 90, minorTicks: 5 };
            }
            if (chrt == "Sankey") {
                ...
                return { width: 400, height: 300, sankey: { node: { colors: colors }, link: { colorMode: "gradient", colors: colors } } };
            }
            if (chrt == "CandlestickChart") {
                return { legend: "none", bar: { groupWidth: "100%" }, candlestick: { fallingColor: { strokeWidth: 0, fill: "#a52714" }, risingColor: { strokeWidth: 0, fill: "#0f9d58" } } };
            }
            return {
                width: 400,
                height: 300,
        ...
        function mixDashboardTilePageSize() {
            ...
            var rows = Math.max(1, Math.floor((availableHeight + 10) / 380));
            return Math.max(4, Math.min(36, columns * rows));
        }

        function setMixDashboardTilePageSize() {
            var hidden = document.getElementById("<%= HiddenDashboardPageSize.ClientID %>");
            if (!hidden) {
                return;
            }

            var size = mixDashboardTilePageSize();
            hidden.value = size;
        ...
        var mixDashboardResizeTimer = null;
        function queueMixDashboardTilePageSize() {
            window.clearTimeout(mixDashboardResizeTimer);
            mixDashboardResizeTimer = window.setTimeout(setMixDashboardTilePageSize, 400);
        }

        if (window.Sys && Sys.Application) {
            Sys.Application.add_load(setMixDashboardTilePageSize);
        } else if (window.addEventListener) {
            window.addEventListener("load", setMixDashboardTilePageSize);
        }

        if (window.addEventListener) {
            window.addEventListener("resize", queueMixDashboardTilePageSize);
        }
    </script>
</head>
<body>
    <form id="form1" runat="server">
        ...
        <asp:UpdatePanel ID="udpMixDashboard" runat="server">
            <ContentTemplate>
                <table>
                    <tr>
                            ...
                            <div style="text-align: center; width: 100%;">
                                ...
                                <div class="dashboard">
                                    <div class="dashboardHeader">
                                        <asp:Label ID="lblHeader" runat="server" CssClass="dashboardTitle" Text="Mixed Dashboard"></asp:Label>
                                        <asp:Label ID="LabelDescription" runat="server" CssClass="dashboardSubTitle" Text="This dashboard contains chart dashboard items and saved analytics tiles."></asp:Label>
                                    </div>
                                    <table class="exportPanel" cellpadding="4" cellspacing="0">
                                        <tr>
                                            ...
                                            <td style="width:25%; vertical-align:middle; text-align:center;">
                                                <asp:Button ID="ButtonExportZip" runat="server" CssClass="ticketbutton" Text="Export as zipped folder" ToolTip="Export dashboard notes, file manifest, PNG chart pictures, and report-view PDF files into one ZIP file." OnClientClick="return prepareDashboardExportImagesAndSubmit(this);" />
                                                <br />
                                                <asp:Button ID="ButtonExportPdf" runat="server" CssClass="ticketbutton" Text="Export as PDF document(s)" Width="190px" ToolTip="Export dashboard notes, manifest, and available visual files as PDF document package." OnClientClick="return prepareDashboardExportImagesAndSubmit(this);" />
                                            </td>
                                        </tr>
                                    </table>
        ...
        <asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="udpMixDashboard">
            <ProgressTemplate>
                <div class="modal">
                    <div class="center">
