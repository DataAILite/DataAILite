...
Partial Class DataReadinessScanner
    ...
    Private Sub BuildAndBindAnalysis()
        ...
        AddRecommendation(output, "Automated Analysis Narratives", ScoreAny(source), "Narratives turn field behavior, data quality, numeric ranges, date fields, and selected focus fields into readable findings and next-step recommendations.", FieldGuidance("Primary Field can emphasize categories, dates, or important business fields", Prefer(Prefer(categoryFields, dateFields), textFields)) & "; " & FieldGuidance("Value Field can provide numeric evidence for the narrative", numericFields) & "; Narrative Focus controls executive summary, quality, field behavior, trends, or exceptions", "AutomatedAnalysisNarratives.aspx", rowCount)
        AddRecommendation(output, "Cross-Report Comparison", If(categoryFields.Count > 0 OrElse numericFields.Count > 0, 72, 25), "Cross-report comparison is useful when another report should be matched to the active report by key fields and compared by counts or numeric values.", FieldGuidance("Key Field should exist in both reports and identify product, customer, region, period, category, or another common dimension", categoryFields, dateFields) & "; " & FieldGuidance("Value Field should be numeric when comparing sums, averages, minimums, or maximums", numericFields) & "; Compare Report ID identifies the second report", "CrossReportComparison.aspx", rowCount)
    ...
    Private Function WhatNextLinks(analysis As String) As String
        Select Case analysis
            ...
            Case "Rule-Based Alerts"
                Return "DataQuality.aspx|Data Quality;AnomalyScoring.aspx|Anomaly Scoring;AutomatedAnalysisNarratives.aspx|Automated Analysis Narratives;CorrelationThreshold.aspx|Correlation Threshold;MapReadines.aspx|Map Readiness"
            Case "Automated Analysis Narratives"
                Return "RuleBasedAlerts.aspx|Rule-Based Alerts;DataQuality.aspx|Data Quality;AuditSummaries.aspx|Audit Summaries"
            Case "Cross-Report Comparison"
                Return "ComparisonReports.aspx|Comparison Reports;Variance.aspx|Variance Analysis;DataDrift.aspx|Data Drift Analysis;AutomatedAnalysisNarratives.aspx|Automated Analysis Narratives"
    ...
    Private Sub BindAnalysisGrid(ByVal dt As DataTable)
        ...
        AnalysisExportSnapshot.Save(Me, "DataReadinessScanner", "Data Readiness Scanner", LabelInfo, GridViewDataReadinessScanner, dt)
