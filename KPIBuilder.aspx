<%@ Page Language="VB" AutoEventWireup="false" CodeFile="KPIBuilder.aspx.vb" Inherits="KPIBuilder" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>KPI Builder</title>
    <style type="text/css">
...
<asp:UpdatePanel ID="udpKPIBuilder" runat="server">
<ContentTemplate>
<table>
...
<tr>
<td style="font-size:x-small; background-color:#e5e5e5; vertical-align:top; width:15%;"><asp:TreeView ID="TreeView1" runat="server" Width="100%" NodeIndent="10" Font-Names="Times New Roman" EnableTheming="True" ImageSet="BulletedList">
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=KPI%20Builder" Target="_blank" CssClass="NodeStyle" Font-Names="Arial">Help</asp:HyperLink>
...
<br /><br />
<div style="text-align:center;"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial">KPI Builder</asp:Label><asp:Label ID="LabelAnalysisSubtitle" runat="server" CssClass="analysisSubtitle" Text="Create simple calculated key performance indicators from selected report fields and review the results by dimension."></asp:Label></div>
<span class="analysisRecommendation">Work Flow suggested: review the complete results in <a class="NodeStyle" href="Variance.aspx">Variance Analysis</a> and <a class="NodeStyle" href="TimeBasedSummaries.aspx">Time Based Summaries</a>.</span>
<br />
<table class="controlpanel" cellpadding="4" cellspacing="0">
<tr><td style="font-weight:bold;">Dimension Field:</td><td><asp:DropDownList ID="DropDownPrimaryField" runat="server" Width="190px" ToolTip="Group the calculated KPI by this field."></asp:DropDownList></td><td style="font-weight:bold;">Operation:</td><td><asp:DropDownList ID="DropDownOperation" runat="server" Width="150px"><asp:ListItem>Ratio</asp:ListItem><asp:ListItem>Difference</asp:ListItem><asp:ListItem>Sum</asp:ListItem><asp:ListItem>Product</asp:ListItem></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Numerator Field:</td><td><asp:DropDownList ID="DropDownValueField" runat="server" Width="190px" ToolTip="First numeric field in the KPI formula."></asp:DropDownList></td><td style="font-weight:bold;">Denominator Field:</td><td><asp:DropDownList ID="DropDownSecondaryValueField" runat="server" Width="190px" ToolTip="Second numeric field in the KPI formula."></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Search:</td><td colspan="3"><asp:TextBox ID="txtSearch" runat="server" Width="420px" ToolTip="Optional text used to filter rows shown in the analysis grid."></asp:TextBox><asp:Button ID="ButtonBuild" runat="server" CssClass="ticketbutton" Text="Build" ToolTip="Build or refresh the page results using the selected controls. The Build action also saves an Excel snapshot of the results in the session temporary folder so it can be included later in Export Packages if needed." /><asp:Button ID="ButtonReset" runat="server" CssClass="ticketbutton" Text="Reset" ToolTip="Clear search and rebuild the default analysis." /><asp:Button ID="ButtonExportCSV" runat="server" CssClass="ticketbutton" Text="CSV" ToolTip="Export the analysis grid to CSV." /><asp:Button ID="ButtonExportExcel" runat="server" CssClass="ticketbutton" Text="Excel" ToolTip="Export the analysis grid to Excel." /><asp:LinkButton OnClientClick="return showWaitingPanel();" ID="lnkKPIBuilderAI" runat="server" CssClass="aiLinkButton" Font-Names="Arial" ToolTip="Ask AI to interpret the analysis grid and method explanation.">AI</asp:LinkButton></td></tr>
</table>
<asp:Label ID="LabelError" runat="server" ForeColor="Red" Font-Names="Arial" Font-Size="Medium"></asp:Label><br />
<asp:Label ID="LabelBuildExportHint" runat="server" ForeColor="DarkGreen" Font-Names="Arial" Font-Size="Small" Text="Click Build to save results in temporary folder for future Export Packages if needed."></asp:Label><br /><asp:Label ID="LabelInfo" runat="server" ForeColor="Black" Font-Names="Arial" Font-Size="Small"></asp:Label><br /><br /><div style="font-family:Arial; font-size:small; padding-bottom:6px;"><asp:LinkButton ID="LinkButtonPrevious" runat="server" Font-Size="Small" OnClick="LinkButtonPrevious_Click">Previous</asp:LinkButton>&nbsp;&nbsp;<asp:Label ID="LabelPageNumberCaption" runat="server" Font-Names="Arial" Font-Size="Small" Text="Page Number"></asp:Label><asp:TextBox ID="TextBoxPageNumber" runat="server" Width="35px" Font-Names="Arial" Font-Size="Small" AutoPostBack="True" OnTextChanged="TextBoxPageNumber_TextChanged"></asp:TextBox><asp:Label ID="LabelPageCount" runat="server" Font-Names="Arial" Font-Size="Small"></asp:Label>&nbsp;&nbsp;<asp:LinkButton ID="LinkButtonNext" runat="server" Font-Size="Small" OnClick="LinkButtonNext_Click">Next</asp:LinkButton></div>
<div style="overflow:auto; max-width:100%;"><asp:GridView ID="GridViewKPIBuilder" runat="server" CssClass="analysisgrid" AutoGenerateColumns="True" GridLines="Both"></asp:GridView></div>
...
<Triggers><asp:PostBackTrigger ControlID="ButtonExportCSV" /><asp:PostBackTrigger ControlID="ButtonExportExcel" /><asp:PostBackTrigger ControlID="lnkKPIBuilderAI" /></Triggers>
</asp:UpdatePanel>
<asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="udpKPIBuilder"><ProgressTemplate><div class="modal"><div class="center"><asp:Image ID="imgProgress" runat="server" ImageAlign="AbsMiddle" ImageUrl="~/Controls/Images/WaitImage2.gif" />Please Wait...</div></div></ProgressTemplate></asp:UpdateProgress>
