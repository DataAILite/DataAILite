...
Partial Class MixDashboard
    ...
    Private Sub MixDashboard_Init(sender As Object, e As EventArgs) Handles Me.Init
        If Session Is Nothing OrElse Session("admin") Is Nothing OrElse Session("admin").ToString() = "" Then
            Response.Redirect("~/Default.aspx?msg=SessionExpired")
        End If
    End Sub

    Private Sub MixDashboard_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Session Is Nothing OrElse Session("admin") Is Nothing OrElse Session("admin").ToString() = "" Then
            Response.Redirect("~/Default.aspx?msg=SessionExpired")
        End If
    ...
    Private Function RequestedDashboardPage() As Integer
        ...
        If Request("page") IsNot Nothing AndAlso Integer.TryParse(Request("page").ToString(), requestedPage) Then Return Math.Max(1, requestedPage)
        Return 1
    End Function

    Private Function RequestedPageSize() As Integer
        Dim requestedSize As Integer = 12
        If Request("ps") IsNot Nothing AndAlso Integer.TryParse(Request("ps").ToString(), requestedSize) Then Return Math.Max(4, Math.Min(36, requestedSize))
        If HiddenDashboardPageSize.Value IsNot Nothing AndAlso Integer.TryParse(HiddenDashboardPageSize.Value, requestedSize) Then Return Math.Max(4, Math.Min(36, requestedSize))
        Return 12
    End Function
    ...
    Private Function BuildTilesHtml(table As DataTable) As String
        ...
        For rowIndex As Integer = startIndex To endIndex - 1
            ...
            If IsAnalyticsRow(row) Then
                AppendAnalyticsTile(sb, row)
            Else
                AppendChartTile(sb, row)
            End If
        Next
    ...
    Private Sub AppendAnalyticsTile(sb As StringBuilder, row As DataRow)
        ...
        Dim tileText As String = "Analytics tile - " & pageName
        If reportId <> "" Then tileText &= " - report " & reportId

        AppendTile(sb, ResolveTileUrl(rawUrl), title, tileText, "Open this saved analytics view with the selected fields, filters, and options.", BuildPreviewHtml(rawUrl, reportId), "open", True, BuildMixDashboardActionUrl("delindx", FieldText(row("Indx")).Trim()))
    End Sub

    Private Sub AppendChartTile(sb As StringBuilder, row As DataRow)
        Dim chartInfo As ChartTileInfo = BuildChartTileInfo(row)
        If chartInfo.Title.Trim() = "" Then chartInfo.Title = If(chartInfo.ChartType = "", "Chart dashboard item", chartInfo.ChartType & " chart")

        sb.Append("<span class=""chartTile"" title=""")
        sb.Append(Server.HtmlEncode(chartInfo.Title))
        sb.Append(""">")

        If chartInfo.Arr.Trim() <> "" Then
            sb.Append("<span class=""chartBox"" id=""")
            sb.Append(Server.HtmlEncode(chartInfo.DivId))
            sb.Append("""></span>")
            sb.Append(BuildChartScript(chartInfo))
        Else
            sb.Append("<span class=""chartBox previewEmpty"">No chart data is available for this dashboard item.</span>")
        End If

        sb.Append("<span class=""chartActions""><a href=""")
        sb.Append(Server.HtmlEncode(BuildMixDashboardActionUrl("maxindx", chartInfo.IndexValue)))
        sb.Append(""" title=""")
        sb.Append(Server.HtmlEncode(ChartMaximizeToolTip(chartInfo)))
        sb.Append(""">maximize</a>")
        sb.Append("<a href=""")
        sb.Append(Server.HtmlEncode(BuildMixDashboardActionUrl("delindx", chartInfo.IndexValue)))
        sb.Append(""" title=""")
        sb.Append(Server.HtmlEncode(ChartDeleteToolTip(chartInfo)))
        sb.Append(""" onclick=""return confirm('Delete this chart from the dashboard?');"">delete from dashboard</a></span>")
        sb.Append("</span>")
    End Sub

    Private Function ChartMaximizeToolTip(info As ChartTileInfo) As String
        Return "Maximize the chart " & info.ChartType & " -" & info.Title & " - report " & DashboardReportTitleText(info.ReportId) & " (" & info.ReportId & ")"
    End Function

    Private Function ChartDeleteToolTip(info As ChartTileInfo) As String
        Return "Delete the chart " & info.Title & " from the dashboard"
    End Function

    Private Class ChartTileInfo
        Public Property IndexValue As String = String.Empty
        Public Property ReportId As String = String.Empty
        Public Property ChartType As String = String.Empty
        Public Property ChartPackage As String = "corechart"
        Public Property Title As String = String.Empty
        Public Property SortText As String = String.Empty
        Public Property Arr As String = String.Empty
        Public Property Region As String = "world"
        Public Property MapName As String = String.Empty
        Public Property MapYesNo As String = String.Empty
        Public Property WhereStatement As String = String.Empty
        Public Property Y1 As String = String.Empty
        Public Property Y2 As String = String.Empty
        Public Property X1 As String = String.Empty
        Public Property X2 As String = String.Empty
        Public Property MultiX As String = String.Empty
        Public Property MultiField As String = String.Empty
        Public Property MultiValue As String = String.Empty
        Public Property Fn As String = String.Empty
        Public Property DivId As String = String.Empty
    End Class

    Private Sub HandleChartTileActions()
        If Request("delindx") IsNot Nothing AndAlso Request("delindx").ToString().Trim() <> "" Then
            DeleteChartTile(Request("delindx").ToString().Trim())
            Response.Redirect(CurrentMixDashboardUrl())
        End If

        If Request("maxindx") IsNot Nothing AndAlso Request("maxindx").ToString().Trim() <> "" Then
            Dim row As DataRow = DashboardRowByIndex(Request("maxindx").ToString().Trim())
            If row IsNot Nothing Then
                Dim chartInfo As ChartTileInfo = BuildChartTileInfo(row)
                Session("arr") = chartInfo.Arr
                Response.Redirect(BuildMaximizeUrl(chartInfo))
            End If
        End If
    End Sub

    Private Function BuildChartTileInfo(row As DataRow) As ChartTileInfo
        Dim info As New ChartTileInfo()
        info.IndexValue = FieldText(row("Indx")).Trim()
        info.ReportId = FieldText(row("ReportID")).Trim()
        info.ChartType = FieldText(row("ChartType")).Trim()
        info.MapName = FieldText(row("MapName")).Trim()
        info.MapYesNo = FieldText(row("MapYesNo")).Trim()
        info.WhereStatement = FieldText(row("WhereStm")).Trim()
        info.Y1 = FieldText(row("y1")).Trim()
        info.X1 = FieldText(row("x1")).Trim()
        info.X2 = FieldText(row("x2")).Trim()
        info.Fn = FieldText(row("fn1")).Trim()
        info.Y2 = FieldText(row("y2")).Trim()
        info.Region = FieldText(row("Prop2")).Trim()
        info.MultiX = FieldText(row("Prop3")).Trim()
        info.MultiField = FieldText(row("Prop4")).Trim()
        info.MultiValue = FieldText(row("Prop5")).Trim()
        If info.Region = "" Then info.Region = "world"

        info.SortText = ChartSortText(info.X1, info.X2)
        info.Title = FieldText(row("GraphTitle")).Trim()
        If info.Title = "" OrElse info.Title.Equals("Message", StringComparison.OrdinalIgnoreCase) Then info.Title = CalculatedChartTitle(info)
        If info.WhereStatement.Trim() <> "" AndAlso Not info.Title.Contains(info.WhereStatement.Replace("^", "")) Then
            info.Title &= " - " & info.WhereStatement.Replace("^", "").Replace("'", "").Replace("[", "").Replace("]", "")
        End If

        info.Arr = DecodeArr(FieldText(row("ARR"))).Trim()
        If info.Arr = "" Then info.Arr = RefreshChartArr(info)
        If info.ChartType.Equals("MapChart", StringComparison.OrdinalIgnoreCase) Then info.ChartType = "Map"
        info.ChartPackage = ChartPackageName(info.ChartType)
        info.DivId = "chart_mix_" & SafeElementSuffix(info.IndexValue)
        Return info
    End Function

    Private Function BuildChartScript(info As ChartTileInfo) As String
        Dim functionName As String = "drawMixChart_" & SafeElementSuffix(info.IndexValue)
        Dim sb As New StringBuilder()
        sb.Append("<script type=""text/javascript"">")
        sb.Append("google.charts.load('current',{packages:['")
        sb.Append(JavaScriptText(info.ChartPackage))
        sb.Append("']});")
        sb.Append("google.charts.setOnLoadCallback(")
        ...
        sb.Append(info.Arr)
        sb.Append("]);")
        sb.Append("var options=mixDashboardChartOptions('")
        sb.Append(JavaScriptText(info.ChartType))
        sb.Append("');")
        sb.Append("options.title='")
        sb.Append(JavaScriptText(info.Title))
        sb.Append("';")
        sb.Append("options.region='")
        sb.Append(JavaScriptText(info.Region))
        sb.Append("';")
        sb.Append("var chart=new google.visualization.")
        sb.Append(JavaScriptChartType(info.ChartType))
    ...
    Private Function RefreshChartArr(info As ChartTileInfo) As String
        If info.IndexValue = "" Then Return ""
        Dim ret As String = String.Empty
        Dim er As String = String.Empty
        Dim chartRegion As String = info.Region
        ret = ExequteSQLquery("UPDATE ourdashboards SET Prop1='0' WHERE Indx=" & SqlNumber(info.IndexValue))
        Dim arrValue As String = UpdateDashboardARR(dashboardName, info.ReportId, info.ChartType, info.MapName, info.MapYesNo, info.X1, info.X2, info.Y1, info.Y2, info.Fn, "", info.WhereStatement, "0", info.MultiX, info.MultiField, info.MultiValue, CInt(Val(info.IndexValue)), chartRegion, Session("UserConnString"), Session("UserConnProvider"), er)
        If arrValue.Trim() <> "" Then
            info.Region = chartRegion
            ret = ExequteSQLquery("UPDATE ourdashboards SET Prop2='" & SqlText(chartRegion) & "', ARR='" & SqlText(EncodeArr(arrValue)) & "', Prop1='1' WHERE Indx=" & SqlNumber(info.IndexValue))
        End If
        Return arrValue
    End Function

    Private Function DashboardRowByIndex(indexValue As String) As DataRow
        Dim ret As String = String.Empty
        Dim sql As String = "SELECT Indx, Dashboard, GraphTitle, ARR, ReportID, ChartType, Comments, MapName, MapYesNo, WhereStm, y1, x1, x2, fn1, y2, Prop2, Prop3, Prop4, Prop5 FROM ourdashboards WHERE UserID='" & SqlText(Session("logon")) & "' AND Dashboard='" & SqlText(dashboardName) & "'" & ReportSqlFilter() & " AND Indx=" & SqlNumber(indexValue)
        Dim dv As DataView = mRecords(sql, ret)
        If ret.Trim() <> "" OrElse dv Is Nothing OrElse dv.Table Is Nothing OrElse dv.Table.Rows.Count = 0 Then Return Nothing
        Return dv.Table.Rows(0)
    End Function

    Private Sub DeleteChartTile(indexValue As String)
        If SqlNumber(indexValue) = "0" AndAlso indexValue.Trim() <> "0" Then Exit Sub
        ExequteSQLquery("DELETE FROM ourdashboards WHERE UserID='" & SqlText(Session("logon")) & "' AND Dashboard='" & SqlText(dashboardName) & "'" & ReportSqlFilter() & " AND Indx=" & SqlNumber(indexValue))
    End Sub

    Private Function BuildMaximizeUrl(info As ChartTileInfo) As String
        Return "ChartGoogleOne.aspx?das=yes&map=" & Server.UrlEncode(info.MapYesNo) & "&mapname=" & Server.UrlEncode(info.MapName) & "&chartregn=" & Server.UrlEncode(info.Region) & "&Report=" & Server.UrlEncode(info.ReportId) & "&y1=" & Server.UrlEncode(info.Y1) & "&y2=" & Server.UrlEncode(info.Y2) & "&fn=" & Server.UrlEncode(info.Fn) & "&x1=" & Server.UrlEncode(info.X1) & "&x2=" & Server.UrlEncode(info.X2) & "&mx=" & Server.UrlEncode(info.MultiX) & "&mf=" & Server.UrlEncode(info.MultiField) & "&mv=" & Server.UrlEncode(info.MultiValue) & "&charttype=" & Server.UrlEncode(info.ChartType) & "&ttl=" & Server.UrlEncode(info.Title) & "&srt=" & Server.UrlEncode(info.SortText) & "&wherestm=" & Server.UrlEncode(info.WhereStatement)
    End Function

    Private Function CurrentMixDashboardUrl() As String
        Dim url As String = "MixDashboard.aspx?dashboard=" & Server.UrlEncode(dashboardName)
    ...
    Private Function BuildMixDashboardActionUrl(actionName As String, indexValue As String) As String
        Return CurrentMixDashboardUrl() & "&" & actionName & "=" & Server.UrlEncode(indexValue)
    End Function

    Private Function CalculatedChartTitle(info As ChartTileInfo) As String
        If info.MapYesNo.Equals("yes", StringComparison.OrdinalIgnoreCase) Then Return info.MapName

        Dim valueText As String = info.Y1
        Dim groupText As String = info.SortText
        If info.Y2 <> "" OrElse info.MultiX <> "" OrElse info.MultiValue <> "" Then
            If info.Y2 <> "" Then valueText = info.Y2
            If info.MultiX <> "" Then groupText = info.MultiX
        End If

        Dim title As String
        Select Case info.Fn
            Case "Value"
                title = "Value of [" & valueText & "] in group by [" & groupText & "]"
            Case "Count"
                title = "Count of records in group by [" & groupText & "]"
            Case "CountDistinct"
                title = "Distinct Count of [" & valueText & "] in group by [" & groupText & "]"
            Case "Sum", "Avg", "StDev", "Max", "Min"
                title = info.Fn & " of [" & valueText & "] in group by [" & groupText & "]"
            Case Else
                title = If(info.ChartType = "", "Chart dashboard item", info.ChartType & " chart")
        End Select

        If info.MultiValue <> "" AndAlso info.MultiField <> "" Then title &= " for value of " & info.MultiField & " in " & info.MultiValue
        Return title
    End Function

    Private Function ChartSortText(x1 As String, x2 As String) As String
        If x1 = x2 OrElse x2.Trim() = "" Then Return x1
        If x1.Trim() = "" Then Return x2
        Return x1 & ", " & x2
    End Function

    Private Function ChartPackageName(chartType As String) As String
        If chartType.Equals("MapChart", StringComparison.OrdinalIgnoreCase) OrElse chartType.Equals("Map", StringComparison.OrdinalIgnoreCase) Then Return "map"
        If chartType.Equals("GeoChart", StringComparison.OrdinalIgnoreCase) Then Return "geochart"
        If chartType.Equals("Gauge", StringComparison.OrdinalIgnoreCase) Then Return "gauge"
        If chartType.Equals("Sankey", StringComparison.OrdinalIgnoreCase) Then Return "sankey"
        Return "corechart"
    End Function

    Private Function JavaScriptChartType(chartType As String) As String
        Dim text As String = If(chartType, "").Trim()
        If text.Equals("MapChart", StringComparison.OrdinalIgnoreCase) Then Return "Map"
        If text = "" Then Return "PieChart"
        Return SafeJavaScriptIdentifier(text)
    End Function

    Private Function SafeJavaScriptIdentifier(value As String) As String
        Dim sb As New StringBuilder()
        For Each ch As Char In If(value, "")
            If Char.IsLetterOrDigit(ch) OrElse ch = "_"c Then sb.Append(ch)
        Next
        If sb.Length = 0 Then Return "PieChart"
        Return sb.ToString()
    End Function

    Private Function SafeElementSuffix(value As String) As String
        Dim sb As New StringBuilder()
        For Each ch As Char In If(value, "")
            If Char.IsLetterOrDigit(ch) OrElse ch = "_"c OrElse ch = "-"c Then sb.Append(ch)
        Next
        If sb.Length = 0 Then Return Guid.NewGuid().ToString("N")
        Return sb.ToString()
    End Function

    Private Function JavaScriptText(value As String) As String
        Return HttpUtility.JavaScriptStringEncode(If(value, ""))
    End Function
    ...
    Private Sub AppendTile(sb As StringBuilder, url As String, title As String, tileText As String, body As String, preview As String, openText As String, previewIsHtml As Boolean, Optional deleteUrl As String = "")
        ...
        sb.Append(Server.HtmlEncode(body))
        sb.Append("</span><span class=""previewBox"">")
        If previewIsHtml Then
            sb.Append(preview)
        Else
            sb.Append(Server.HtmlEncode(preview))
        End If
        sb.Append("</span><span class=""openText""><a href=""")
        sb.Append(Server.HtmlEncode(url))
        sb.Append(""" title=""Open ")
        sb.Append(Server.HtmlEncode(title))
        sb.Append(""">")
        sb.Append(Server.HtmlEncode(openText))
        sb.Append("</a>")
        If deleteUrl.Trim() <> "" Then
            sb.Append("&nbsp;&nbsp;&nbsp;&nbsp;<a href=""")
            sb.Append(Server.HtmlEncode(deleteUrl))
            sb.Append(""" title=""Delete this tile from the dashboard"" onclick=""return confirm('Delete this tile from the dashboard?');"">delete from dashboard</a>")
        End If
        sb.Append("</span></div>")
    End Sub
    ...
    Private Function IsAnalyticsRow(row As DataRow) As Boolean
