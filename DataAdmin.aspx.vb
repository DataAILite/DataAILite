...
Partial Class DataAdmin
    ...
    Private Function BuildCohortPreviewHtml(dv As DataView) As String
        If Not HasPreviewData(dv) Then Return EmptyPreview("No report data available.")

        Dim entityCol As DataColumn = FirstTextColumn(dv.Table)
        Dim dateCol As DataColumn = FirstDateColumn(dv.Table)
        If entityCol Is Nothing OrElse dateCol Is Nothing Then Return BuildTimeBasedPreviewHtml(dv)

        Dim firstPeriod As New Dictionary(Of String, String)(StringComparer.OrdinalIgnoreCase)
        Dim firstDate As New Dictionary(Of String, DateTime)(StringComparer.OrdinalIgnoreCase)
        Dim cohortCounts As New Dictionary(Of String, Integer)(StringComparer.OrdinalIgnoreCase)

        For i As Integer = 0 To dv.Count - 1
            Dim entityText As String = FieldText(dv(i)(entityCol.ColumnName)).Trim()
            Dim activityDate As DateTime
            If entityText = "" OrElse Not DateTime.TryParse(FieldText(dv(i)(dateCol.ColumnName)), activityDate) Then Continue For

            If Not firstDate.ContainsKey(entityText) OrElse activityDate < firstDate(entityText) Then
                firstDate(entityText) = activityDate
                firstPeriod(entityText) = activityDate.ToString("yyyy-MM", CultureInfo.InvariantCulture)
            End If
        Next

        For Each pair As KeyValuePair(Of String, String) In firstPeriod
            If Not cohortCounts.ContainsKey(pair.Value) Then cohortCounts(pair.Value) = 0
            cohortCounts(pair.Value) += 1
        Next

        Dim periods As New List(Of String)(cohortCounts.Keys)
        periods.Sort()

        Dim rows As New List(Of String())()
        For i As Integer = 0 To Math.Min(PreviewRows, periods.Count) - 1
            rows.Add(New String() {periods(i), cohortCounts(periods(i)).ToString(), entityCol.ColumnName})
        Next

        If rows.Count = 0 Then Return BuildTimeBasedPreviewHtml(dv)
        Return RenderPreviewTable(New String() {"Cohort", "Entities", "Entity Field"}, rows)
    End Function

    Private Function BuildFunnelPreviewHtml(dv As DataView) As String
        If Not HasPreviewData(dv) Then Return EmptyPreview("No report data available.")

        Dim stageCol As DataColumn = FirstTextColumn(dv.Table)
        If stageCol Is Nothing Then Return BuildGroupsPreviewHtml(dv)

        Dim counts As Dictionary(Of String, Integer) = GroupCounts(dv, stageCol)
        Dim records As New List(Of CountRecord)()
        For Each pair As KeyValuePair(Of String, Integer) In counts
            records.Add(New CountRecord(pair.Key, pair.Value))
        Next
        records.Sort(AddressOf CompareCountRecordsDescending)

        Dim firstCount As Double = 0
        If records.Count > 0 Then firstCount = records(0).Value

        Dim rows As New List(Of String())()
        For i As Integer = 0 To Math.Min(PreviewRows, records.Count) - 1
            Dim conversion As Double = 0
            If firstCount > 0 Then conversion = records(i).Value * 100 / firstCount
            rows.Add(New String() {(i + 1).ToString(), records(i).Text, FormatPreviewNumber(records(i).Value), FormatPreviewNumber(conversion) & "%"})
        Next

        If rows.Count = 0 Then Return BuildGroupsPreviewHtml(dv)
        Return RenderPreviewTable(New String() {"Step", "Stage", "Records", "Conversion"}, rows)
    End Function

    Private Function BuildABCParetoPreviewHtml(dv As DataView) As String
        If Not HasPreviewData(dv) Then Return EmptyPreview("No report data available.")

        Dim groupCol As DataColumn = FirstTextColumn(dv.Table)
        Dim valueCol As DataColumn = FirstNumericColumn(dv.Table)
        If groupCol Is Nothing Then Return BuildRankingPreviewHtml(dv)
        ...
        Dim rows As New List(Of String())()
        For i As Integer = 0 To Math.Min(PreviewRows, records.Count) - 1
            cumulative += records(i).Value
            Dim cumulativePercent As Double = 0
            If grandTotal <> 0 Then cumulativePercent = cumulative * 100 / grandTotal
            Dim abcClass As String = If(cumulativePercent <= 80, "A", If(cumulativePercent <= 95, "B", "C"))
            rows.Add(New String() {records(i).Text, FormatPreviewNumber(records(i).Value), FormatPreviewNumber(cumulativePercent) & "%", abcClass})
        Next

        If rows.Count = 0 Then Return BuildRankingPreviewHtml(dv)
        Return RenderPreviewTable(New String() {groupCol.ColumnName, "Value", "Cumulative", "Class"}, rows)
    End Function

    Private Function BuildDataDriftPreviewHtml(dv As DataView) As String
        If Not HasPreviewData(dv) Then Return EmptyPreview("No report data available.")

        Dim textCols As List(Of DataColumn) = TextColumns(dv.Table)
        If textCols.Count < 2 Then Return BuildGroupsPreviewHtml(dv)

        Dim compareCol As DataColumn = textCols(0)
        Dim segmentCol As DataColumn = textCols(1)
        Dim segments As List(Of String) = TopValues(dv, segmentCol, 2)
        If segments.Count < 2 Then Return BuildGroupsPreviewHtml(dv)

        Dim values As List(Of String) = TopValues(dv, compareCol, PreviewRows)
        Dim rows As New List(Of String())()
        For i As Integer = 0 To values.Count - 1
            Dim baseCount As Integer = FilteredCount(dv, compareCol, values(i), segmentCol, segments(0))
            Dim compareCount As Integer = FilteredCount(dv, compareCol, values(i), segmentCol, segments(1))
            rows.Add(New String() {values(i), baseCount.ToString(), compareCount.ToString(), (compareCount - baseCount).ToString()})
        Next
    ...
    Private Function BuildKPIBuilderPreviewHtml(dv As DataView) As String
        ...
        Dim groupCol As DataColumn = FirstTextColumn(dv.Table)
        Dim numericCols As List(Of DataColumn) = NumericColumns(dv.Table)
        If groupCol Is Nothing OrElse numericCols.Count < 2 Then Return BuildRankingPreviewHtml(dv)

        Dim numeratorTotals As Dictionary(Of String, Double) = GroupTotals(dv, groupCol, numericCols(0))
        Dim denominatorTotals As Dictionary(Of String, Double) = GroupTotals(dv, groupCol, numericCols(1))
        Dim groupValues As List(Of String) = TopValues(dv, groupCol, PreviewRows)

        Dim rows As New List(Of String())()
        For i As Integer = 0 To groupValues.Count - 1
            Dim denominatorValue As Double = 0
            If denominatorTotals.ContainsKey(groupValues(i)) Then denominatorValue = denominatorTotals(groupValues(i))
            Dim numeratorValue As Double = 0
            If numeratorTotals.ContainsKey(groupValues(i)) Then numeratorValue = numeratorTotals(groupValues(i))
            Dim kpiValue As Double = 0
            If denominatorValue <> 0 Then kpiValue = numeratorValue / denominatorValue
            rows.Add(New String() {groupValues(i), FormatPreviewNumber(numeratorValue), FormatPreviewNumber(denominatorValue), FormatPreviewNumber(kpiValue)})
        Next

        If rows.Count = 0 Then Return BuildRankingPreviewHtml(dv)
        Return RenderPreviewTable(New String() {groupCol.ColumnName, numericCols(0).ColumnName, numericCols(1).ColumnName, "Ratio"}, rows)
    End Function

    Private Function BuildDataDictionaryPreviewHtml(dv As DataView) As String
        If Not HasPreviewData(dv) Then Return EmptyPreview("No report data available.")
        ...
        For colIndex As Integer = 0 To Math.Min(PreviewRows, dv.Table.Columns.Count) - 1
            ...
            rows.Add(New String() {col.ColumnName, FriendlyType(col), BlankCount(dv, col).ToString(), DistinctCount(dv, col).ToString()})
        Next
