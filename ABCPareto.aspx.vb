...
Partial Class ABCPareto
    Inherits System.Web.UI.Page

    Private Const AnalysisGridPageSize As Integer = 50

    Private Sub ABCPareto_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        If Session("REPTITLE") IsNot Nothing AndAlso Session("REPTITLE").ToString().Trim() <> "" Then lblHeader.Text = Session("REPTITLE").ToString() & " - ABC Pareto Analysis"
        HyperLinkHelp.NavigateUrl = "DataAIHelp.aspx?hilt=ABC%20Pareto%20Analysis"
    End Sub

    Private Sub ABCPareto_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Session Is Nothing OrElse Session("admin") Is Nothing OrElse Session("admin").ToString() = "" Then Response.Redirect("~/Default.aspx?msg=SessionExpired")
        If Not IsPostBack Then
            LoadReportData()
            FillFieldLists()
            BuildAndBindAnalysis()
        ElseIf Session("ABCParetoTable") IsNot Nothing Then
            BindAnalysisGrid(CType(Session("ABCParetoTable"), DataTable))
        End If
    End Sub
    ...
    Private Function LoadReportData() As DataTable
        ...
        If repid.Trim() = "" Then
            ...
            If existingTable IsNot Nothing AndAlso existingTable.Rows.Count > 0 Then
                Session("ABCParetoTableSource") = existingTable
        ...
        If dv Is Nothing OrElse dv.Table Is Nothing OrElse dv.Table.Rows.Count = 0 Then
            ...
            Session("ABCParetoTableSource") = Nothing
            Return Nothing
        End If
        Session("ABCParetoTableSource") = dv.Table
        Return dv.Table
    End Function

    Private Function GetSourceTable() As DataTable
        If Session("ABCParetoTableSource") Is Nothing Then Return LoadReportData()
        Return CType(Session("ABCParetoTableSource"), DataTable)
    End Function
    ...
    Private Sub lnkABCParetoAI_Click(sender As Object, e As EventArgs) Handles lnkABCParetoAI.Click
        Dim dt As DataTable = TryCast(Session("ABCParetoTable"), DataTable)
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            BuildAndBindAnalysis()
            dt = TryCast(Session("ABCParetoTable"), DataTable)
        End If
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            LabelError.Text = "No abc pareto analysis results to send to AI."
            Exit Sub
        End If
        Dim aiTable As DataTable = GridTableForAI(dt)
        Session("dataTable") = aiTable
        Session("OriginalDataTable") = Session("dataTable")
        Session("DataToChatAI") = ExportToCSVtext(aiTable, Chr(9), "ABC Pareto Analysis", "")
        Session("QuestionToAI") = BuildAnalysisQuestion("Interpret this abc pareto analysis grid. Explain the strongest findings, unusual records, and business meaning of the results.")
    ...
    Private Sub BuildAndBindAnalysis()
        ...
        output.Columns.Add("Dimension", GetType(String))
        output.Columns.Add("Records", GetType(Integer))
        output.Columns.Add("Value", GetType(Double))
        output.Columns.Add("Share %", GetType(Double))
        output.Columns.Add("Cumulative %", GetType(Double))
        output.Columns.Add("ABC Class", GetType(String))
        ...
        Dim dimCol As String = DropDownPrimaryField.SelectedValue
        ...
        For Each row As DataRow In source.Rows
            ...
            Dim dimText As String = FieldText(row(dimCol)).Trim()
            If dimText = "" Then dimText = "(blank)"
            If Not sums.ContainsKey(dimText) Then sums(dimText) = 0 : counts(dimText) = 0
            counts(dimText) += 1
            If DropDownAggregate.SelectedValue = "Count" OrElse valueCol = "" Then sums(dimText) += 1 Else sums(dimText) += NumericValue(row(valueCol))
        Next
        If DropDownAggregate.SelectedValue = "Average" Then
            Dim keys2 As New List(Of String)(sums.Keys)
            For Each k As String In keys2
                If counts(k) > 0 Then sums(k) = sums(k) / counts(k)
            Next
        End If
        Dim items As New List(Of KeyValuePair(Of String, Double))(sums)
        items.Sort(Function(a, b) b.Value.CompareTo(a.Value))
        Dim total As Double = 0
        For Each item As KeyValuePair(Of String, Double) In items
            total += item.Value
        Next
        Dim cumulative As Double = 0
        For Each item As KeyValuePair(Of String, Double) In items
            cumulative += item.Value
            Dim share As Double = If(total = 0, 0, item.Value * 100.0 / total)
            Dim cumPct As Double = If(total = 0, 0, cumulative * 100.0 / total)
            Dim r As DataRow = output.NewRow()
            r("Dimension") = item.Key
            r("Records") = counts(item.Key)
            r("Value") = Math.Round(item.Value, 4)
            r("Share %") = Math.Round(share, 2)
            r("Cumulative %") = Math.Round(cumPct, 2)
            r("ABC Class") = If(cumPct <= 80, "A", If(cumPct <= 95, "B", "C"))
            r("FilterId") = RegisterAnalysisFilter(FilterEquals(dimCol, item.Key))
            output.Rows.Add(r)
        Next
        BindAnalysisGrid(output)
    End Sub

    Private Sub GridViewABCPareto_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridViewABCPareto.RowDataBound
        If e.Row.Cells.Count = 0 Then Exit Sub
        Dim dt As DataTable = TryCast(Session("ABCParetoTable"), DataTable)
        If dt Is Nothing Then Exit Sub
        HideFilterColumnsInRow(dt, e.Row)
        If e.Row.RowType <> DataControlRowType.DataRow Then Exit Sub
        AddRecordLink(e.Row, dt, "Records", "FilterId", "abcfilter")
    End Sub

    Private Sub BindAnalysisGrid(ByVal dt As DataTable)
        Session("ABCParetoTable") = dt
        Session(AnalysisGridSessionKey()) = dt
        If dt Is Nothing Then
            GridViewABCPareto.AllowPaging = False
            GridViewABCPareto.PageIndex = 0
            GridViewABCPareto.DataSource = Nothing
            GridViewABCPareto.DataBind()
            UpdateAnalysisPager(Nothing)
            SetAnalysisExplanationLabels()
            Return
        End If
        GridViewABCPareto.AllowPaging = (dt.Rows.Count > AnalysisGridPageSize)
        GridViewABCPareto.PageSize = AnalysisGridPageSize
        If Not GridViewABCPareto.AllowPaging Then GridViewABCPareto.PageIndex = 0
        GridViewABCPareto.DataSource = dt
        GridViewABCPareto.DataBind()
    ...
    Private Sub UpdateAnalysisPager(ByVal dt As DataTable)
        ...
        LinkButtonPrevious.Visible = hasPages AndAlso GridViewABCPareto.PageIndex > 0
        LinkButtonNext.Visible = hasPages AndAlso GridViewABCPareto.PageIndex < (GridViewABCPareto.PageCount - 1)
        LabelPageNumberCaption.Visible = hasPages
        TextBoxPageNumber.Visible = hasPages
        LabelPageCount.Visible = hasPages
        If hasPages Then
            TextBoxPageNumber.Text = (GridViewABCPareto.PageIndex + 1).ToString()
            LabelPageCount.Text = " of " & GridViewABCPareto.PageCount.ToString()
        Else
    ...
    Protected Sub LinkButtonPrevious_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If GridViewABCPareto.PageIndex > 0 Then GridViewABCPareto.PageIndex -= 1
        BindAnalysisGrid(dt)
    End Sub
    ...
    Protected Sub LinkButtonNext_Click(ByVal sender As Object, ByVal e As EventArgs)
        Dim dt As DataTable = TryCast(Session(AnalysisGridSessionKey()), DataTable)
        If dt Is Nothing Then Return
        If GridViewABCPareto.PageIndex < (GridViewABCPareto.PageCount - 1) Then GridViewABCPareto.PageIndex += 1
        BindAnalysisGrid(dt)
    End Sub
    ...
    Protected Sub TextBoxPageNumber_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Integer.TryParse(TextBoxPageNumber.Text, requestedPage) Then
            ...
            GridViewABCPareto.PageIndex = requestedPage - 1
        End If
        BindAnalysisGrid(dt)
    End Sub
    ...
    Private Sub ExportAnalysis(formatName As String)
        ...
        End If
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            LabelError.Text = "No abc pareto analysis results to export."
            Exit Sub
        End If
        Dim publicTable As DataTable = GridTableForAI(dt)
        Dim fileName As String = "ABCPareto_" & DateTime.Now.ToString("yyyyMMddHHmmss")
        Response.Clear()
        If formatName = "csv" Then
            Response.ContentType = "text/csv"
            Response.AddHeader("Content-Disposition", "attachment; filename=" & fileName & ".csv")
            Response.Write(ExportToCSVtext(publicTable, ",", "ABC Pareto Analysis", ""))
        Else
            Response.ContentType = "application/vnd.ms-excel"
            Response.AddHeader("Content-Disposition", "attachment; filename=" & fileName & ".xls")
            Response.Write(ExportToCSVtext(publicTable, Chr(9), "ABC Pareto Analysis", ""))
        End If
        Response.End()
    End Sub
    ...
    Private Function RegisterAnalysisFilter(filterExpression As String) As String
        Dim filters As Dictionary(Of String, String) = TryCast(Session("ABCParetoFilters"), Dictionary(Of String, String))
        ...
        Session("ABCParetoFilters") = filters
        Return filterId
    End Function
