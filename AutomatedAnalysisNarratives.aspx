<%@ Page Language="VB" AutoEventWireup="false" CodeFile="AutomatedAnalysisNarratives.aspx.vb" Inherits="AutomatedAnalysisNarratives" %>
<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Automated Analysis Narratives</title>
    <style type="text/css">
...
<asp:UpdatePanel ID="udpNarratives" runat="server">
<ContentTemplate>
<table>
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=Automated%20Analysis%20Narratives" Target="_blank" CssClass="NodeStyle" Font-Names="Arial">Help</asp:HyperLink>
...
<br /><br />
<div style="text-align:center;"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial">Automated Analysis Narratives</asp:Label><asp:Label ID="LabelAnalysisSubtitle" runat="server" CssClass="analysisSubtitle" Text="Generate structured narrative findings from the current report data, including data quality notes, field behavior, trends, exceptions, and recommended follow-up analysis."></asp:Label></div>
<span class="analysisRecommendation">Work Flow suggested: review narrative findings with <a class="NodeStyle" href="DataReadinessScanner.aspx">Data Readiness Scanner</a>, <a class="NodeStyle" href="RuleBasedAlerts.aspx">Rule-Based Alerts</a>, and <a class="NodeStyle" href="DataQuality.aspx">Data Quality</a>.</span>
<br />
<table class="controlpanel" cellpadding="4" cellspacing="0" style="min-width:1120px;">

<tr><td style="font-weight:bold;">Narrative Focus:</td><td><asp:DropDownList ID="DropDownFocus" runat="server" Width="220px" ToolTip="Select the type of automated narrative to produce."><asp:ListItem>Executive Summary</asp:ListItem><asp:ListItem>Data Quality</asp:ListItem><asp:ListItem>Field Behavior</asp:ListItem><asp:ListItem>Trends and Movement</asp:ListItem><asp:ListItem>Risk and Exceptions</asp:ListItem></asp:DropDownList></td><td style="font-weight:bold;">Detail Level:</td><td><asp:DropDownList ID="DropDownDetail" runat="server" Width="150px" ToolTip="Controls how much evidence is included in each narrative row."><asp:ListItem>Short</asp:ListItem><asp:ListItem Selected="True">Standard</asp:ListItem><asp:ListItem>Detailed</asp:ListItem></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Primary Field:</td><td><asp:DropDownList ID="DropDownPrimaryField" runat="server" Width="220px" ToolTip="Optional category, group, date, or field to emphasize in the narrative."></asp:DropDownList></td><td style="font-weight:bold;">Value Field:</td><td><asp:DropDownList ID="DropDownValueField" runat="server" Width="220px" ToolTip="Optional numeric value field used for narrative evidence."></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Search:</td><td colspan="3"><asp:TextBox ID="txtSearch" runat="server" Width="420px" ToolTip="Optional text used to filter records before narrative generation."></asp:TextBox><asp:Button ID="ButtonBuild" runat="server" CssClass="ticketbutton" Text="Build" OnClientClick="return showWaitingPanel();" ToolTip="Build or refresh the page results using the selected controls. The Build action also saves an Excel snapshot of the results in the session temporary folder so it can be included later in Export Packages if needed." /><asp:Button ID="ButtonReset" runat="server" CssClass="ticketbutton" Text="Reset" ToolTip="Clear selected options and rebuild the default analysis." /><asp:Button ID="ButtonExportCSV" runat="server" CssClass="ticketbutton" Text="CSV" ToolTip="Export the analysis grid to CSV." /><asp:Button ID="ButtonExportExcel" runat="server" CssClass="ticketbutton" Text="Excel" ToolTip="Export the analysis grid to Excel." /><asp:LinkButton OnClientClick="return showWaitingPanel();" ID="lnkNarrativesAI" runat="server" CssClass="aiLinkButton" Font-Names="Arial" ToolTip="Ask AI to expand or explain the automated narrative rows.">AI</asp:LinkButton></td></tr>
</table>
...
<div style="overflow:auto; max-width:100%;"><asp:GridView ID="GridViewNarratives" runat="server" CssClass="analysisgrid" AutoGenerateColumns="True" GridLines="Both"></asp:GridView></div>
...
<Triggers><asp:PostBackTrigger ControlID="ButtonExportCSV" /><asp:PostBackTrigger ControlID="ButtonExportExcel" /><asp:PostBackTrigger ControlID="lnkNarrativesAI" /></Triggers>
</asp:UpdatePanel>
<asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="udpNarratives"><ProgressTemplate><div class="modal"><div class="center"><asp:Image ID="imgProgress" runat="server" ImageAlign="AbsMiddle" ImageUrl="~/Controls/Images/WaitImage2.gif" />Please Wait...</div></div></ProgressTemplate></asp:UpdateProgress>
