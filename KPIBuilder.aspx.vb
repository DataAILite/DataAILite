...
Partial Class KPIBuilder
    ...
    Private Sub KPIBuilder_Init(sender As Object, e As EventArgs) Handles Me.Init
        ...
        If Session("REPTITLE") IsNot Nothing AndAlso Session("REPTITLE").ToString().Trim() <> "" Then lblHeader.Text = Session("REPTITLE").ToString() & " - KPI Builder"
        HyperLinkHelp.NavigateUrl = "DataAIHelp.aspx?hilt=KPI%20Builder"
    End Sub

    Private Sub KPIBuilder_Load(sender As Object, e As EventArgs) Handles Me.Load
        If Session Is Nothing OrElse Session("admin") Is Nothing OrElse Session("admin").ToString() = "" Then Response.Redirect("~/Default.aspx?msg=SessionExpired")
    ...
    Private Function LoadReportData() As DataTable
        ...
        If repid.Trim() = "" Then
            ...
            If existingTable IsNot Nothing AndAlso existingTable.Rows.Count > 0 Then
                Session("KPIBuilderTableSource") = existingTable
        ...
        If dv Is Nothing OrElse dv.Table Is Nothing OrElse dv.Table.Rows.Count = 0 Then
            ...
            Session("KPIBuilderTableSource") = Nothing
            Return Nothing
        End If
        Session("KPIBuilderTableSource") = dv.Table
        Return dv.Table
    End Function

    Private Function GetSourceTable() As DataTable
        If Session("KPIBuilderTableSource") Is Nothing Then Return LoadReportData()
        Return CType(Session("KPIBuilderTableSource"), DataTable)
    End Function
    ...
    Private Sub ButtonBuild_Click(sender As Object, e As EventArgs) Handles ButtonBuild.Click
        GridViewKPIBuilder.PageIndex = 0
        BuildAndBindAnalysis()
    End Sub
    ...
    Private Sub lnkKPIBuilderAI_Click(sender As Object, e As EventArgs) Handles lnkKPIBuilderAI.Click
        Dim dt As DataTable = TryCast(Session("KPIBuilderTable"), DataTable)
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            BuildAndBindAnalysis()
            dt = TryCast(Session("KPIBuilderTable"), DataTable)
        End If
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            LabelError.Text = "No kpi builder results to send to AI."
            Exit Sub
        End If
        Dim aiTable As DataTable = GridTableForAI(dt)
        Session("dataTable") = aiTable
        Session("OriginalDataTable") = Session("dataTable")
        Session("DataToChatAI") = ExportToCSVtext(aiTable, Chr(9), "KPI Builder", "")
        Session("QuestionToAI") = BuildAnalysisQuestion("Interpret this kpi builder grid. Explain the strongest findings, unusual records, and business meaning of the results.")
    ...
    Private Sub BuildAndBindAnalysis()
        ...
        output.Columns.Add("Numerator", GetType(Double))
        output.Columns.Add("Denominator", GetType(Double))
        output.Columns.Add("Operation", GetType(String))
        output.Columns.Add("KPI Value", GetType(Double))
        ...
        Dim numCol As String = DropDownValueField.SelectedValue
        Dim denCol As String = DropDownSecondaryValueField.SelectedValue
        Dim nums As New Dictionary(Of String, Double)(StringComparer.OrdinalIgnoreCase)
        Dim dens As New Dictionary(Of String, Double)(StringComparer.OrdinalIgnoreCase)
        ...
        For Each row As DataRow In source.Rows
            ...
            If Not nums.ContainsKey(dimText) Then nums(dimText) = 0 : dens(dimText) = 0 : counts(dimText) = 0
            counts(dimText) += 1
            nums(dimText) += If(numCol = "", 1, NumericValue(row(numCol)))
            dens(dimText) += If(denCol = "", 1, NumericValue(row(denCol)))
        Next
        For Each k As String In nums.Keys
            Dim result As Double = 0
            Select Case DropDownOperation.SelectedValue
                Case "Difference" : result = nums(k) - dens(k)
                Case "Sum" : result = nums(k) + dens(k)
                Case "Product" : result = nums(k) * dens(k)
                Case Else : If dens(k) <> 0 Then result = nums(k) / dens(k)
            End Select
            Dim r As DataRow = output.NewRow()
            r("Dimension") = k
            r("Records") = counts(k)
            r("Numerator") = Math.Round(nums(k), 4)
            r("Denominator") = Math.Round(dens(k), 4)
            r("Operation") = DropDownOperation.SelectedValue
            r("KPI Value") = Math.Round(result, 4)
            r("FilterId") = RegisterAnalysisFilter(FilterEquals(dimCol, k))
            output.Rows.Add(r)
        Next
        output.DefaultView.Sort = "KPI Value DESC"
        BindAnalysisGrid(output.DefaultView.ToTable())
    End Sub

    Private Sub GridViewKPIBuilder_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridViewKPIBuilder.RowDataBound
        If e.Row.Cells.Count = 0 Then Exit Sub
        Dim dt As DataTable = TryCast(Session("KPIBuilderTable"), DataTable)
        If dt Is Nothing Then Exit Sub
        HideFilterColumnsInRow(dt, e.Row)
        If e.Row.RowType <> DataControlRowType.DataRow Then Exit Sub
        AddRecordLink(e.Row, dt, "Records", "FilterId", "kpifilter")
    End Sub

    Private Sub BindAnalysisGrid(ByVal dt As DataTable)
        Session("KPIBuilderTable") = dt
        Session(AnalysisGridSessionKey()) = dt
        If dt Is Nothing Then
            GridViewKPIBuilder.AllowPaging = False
            GridViewKPIBuilder.PageIndex = 0
            GridViewKPIBuilder.DataSource = Nothing
            GridViewKPIBuilder.DataBind()
            UpdateAnalysisPager(Nothing)
            SetAnalysisExplanationLabels()
            Return
        End If
        GridViewKPIBuilder.AllowPaging = (dt.Rows.Count > AnalysisGridPageSize)
        GridViewKPIBuilder.PageSize = AnalysisGridPageSize
        If Not GridViewKPIBuilder.AllowPaging Then GridViewKPIBuilder.PageIndex = 0
        GridViewKPIBuilder.DataSource = dt
        GridViewKPIBuilder.DataBind()
    ...
    Private Sub UpdateAnalysisPager(ByVal dt As DataTable)
        ...
        LinkButtonPrevious.Visible = hasPages AndAlso GridViewKPIBuilder.PageIndex > 0
        LinkButtonNext.Visible = hasPages AndAlso GridViewKPIBuilder.PageIndex < (GridViewKPIBuilder.PageCount - 1)
        LabelPageNumberCaption.Visible = hasPages
        TextBoxPageNumber.Visible = hasPages
        LabelPageCount.Visible = hasPages
        If hasPages Then
            TextBoxPageNumber.Text = (GridViewKPIBuilder.PageIndex + 1).ToString()
            LabelPageCount.Text = " of " & GridViewKPIBuilder.PageCount.ToString()
        Else
    ...
    Protected Sub LinkButtonPrevious_Click(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If GridViewKPIBuilder.PageIndex > 0 Then GridViewKPIBuilder.PageIndex -= 1
        BindAnalysisGrid(dt)
    End Sub
    ...
    Protected Sub LinkButtonNext_Click(ByVal sender As Object, ByVal e As EventArgs)
        Dim dt As DataTable = TryCast(Session(AnalysisGridSessionKey()), DataTable)
        If dt Is Nothing Then Return
        If GridViewKPIBuilder.PageIndex < (GridViewKPIBuilder.PageCount - 1) Then GridViewKPIBuilder.PageIndex += 1
        BindAnalysisGrid(dt)
    End Sub
    ...
    Protected Sub TextBoxPageNumber_TextChanged(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If Integer.TryParse(TextBoxPageNumber.Text, requestedPage) Then
            ...
            GridViewKPIBuilder.PageIndex = requestedPage - 1
        End If
        BindAnalysisGrid(dt)
    End Sub
    ...
    Private Sub ExportAnalysis(formatName As String)
        ...
        End If
        If dt Is Nothing OrElse dt.Rows.Count = 0 Then
            LabelError.Text = "No kpi builder results to export."
            Exit Sub
        End If
        Dim publicTable As DataTable = GridTableForAI(dt)
        Dim fileName As String = "KPIBuilder_" & DateTime.Now.ToString("yyyyMMddHHmmss")
        Response.Clear()
        If formatName = "csv" Then
            Response.ContentType = "text/csv"
            Response.AddHeader("Content-Disposition", "attachment; filename=" & fileName & ".csv")
            Response.Write(ExportToCSVtext(publicTable, ",", "KPI Builder", ""))
        Else
            Response.ContentType = "application/vnd.ms-excel"
            Response.AddHeader("Content-Disposition", "attachment; filename=" & fileName & ".xls")
            Response.Write(ExportToCSVtext(publicTable, Chr(9), "KPI Builder", ""))
        End If
        Response.End()
    End Sub
    ...
    Private Function RegisterAnalysisFilter(filterExpression As String) As String
        Dim filters As Dictionary(Of String, String) = TryCast(Session("KPIBuilderFilters"), Dictionary(Of String, String))
