<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Cohort.aspx.vb" Inherits="Cohort" %>
<!DOCTYPE html>
...
<asp:UpdatePanel ID="udpCohort" runat="server">
<ContentTemplate>
<table>
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=Cohort" Target="_blank" CssClass="NodeStyle" Font-Names="Arial">Help</asp:HyperLink>
...
<br /><br />
<div style="text-align:center;"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial">Cohort Analysis</asp:Label><asp:Label ID="LabelAnalysisSubtitle" runat="server" CssClass="analysisSubtitle" Text="Group records by first activity period and measure later-period activity, retention, records, and value."></asp:Label></div>
<span class="analysisRecommendation">Work Flow suggested: review the complete results in <a class="NodeStyle" href="MarketChurn.aspx">Market Churn</a> and <a class="NodeStyle" href="TimeSeries.aspx">Time Series</a>.</span>
<br />
<table class="controlpanel" cellpadding="4" cellspacing="0">
<tr><td style="font-weight:bold;">Entity Field:</td><td><asp:DropDownList ID="DropDownPrimaryField" runat="server" Width="190px" ToolTip="Field that identifies the customer, account, product, location, or other item tracked over time."></asp:DropDownList></td><td style="font-weight:bold;">Date Field:</td><td><asp:DropDownList ID="DropDownDateField" runat="server" Width="190px" ToolTip="Date used to find first activity and later activity periods."></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Value Field:</td><td><asp:DropDownList ID="DropDownValueField" runat="server" Width="190px" ToolTip="Numeric field to summarize for cohort value."></asp:DropDownList></td><td style="font-weight:bold;">Period:</td><td><asp:DropDownList ID="DropDownPeriod" runat="server" Width="120px"><asp:ListItem>Month</asp:ListItem><asp:ListItem>Quarter</asp:ListItem><asp:ListItem>Year</asp:ListItem></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Search:</td><td colspan="3"><asp:TextBox ID="txtSearch" runat="server" Width="420px" ToolTip="Optional text used to filter rows shown in the analysis grid."></asp:TextBox><asp:Button ID="ButtonBuild" runat="server" CssClass="ticketbutton" Text="Build" ToolTip="Build or refresh the page results using the selected controls. The Build action also saves an Excel snapshot of the results in the session temporary folder so it can be included later in Export Packages if needed." /><asp:Button ID="ButtonReset" runat="server" CssClass="ticketbutton" Text="Reset" ToolTip="Clear search and rebuild the default analysis." /><asp:Button ID="ButtonExportCSV" runat="server" CssClass="ticketbutton" Text="CSV" ToolTip="Export the analysis grid to CSV." /><asp:Button ID="ButtonExportExcel" runat="server" CssClass="ticketbutton" Text="Excel" ToolTip="Export the analysis grid to Excel." /><asp:LinkButton OnClientClick="return showWaitingPanel();" ID="lnkCohortAI" runat="server" CssClass="aiLinkButton" Font-Names="Arial" ToolTip="Ask AI to interpret the analysis grid and method explanation.">AI</asp:LinkButton></td></tr>
</table>
...
<div style="overflow:auto; max-width:100%;"><asp:GridView ID="GridViewCohort" runat="server" CssClass="analysisgrid" AutoGenerateColumns="True" GridLines="Both"></asp:GridView></div>
...
<Triggers><asp:PostBackTrigger ControlID="ButtonExportCSV" /><asp:PostBackTrigger ControlID="ButtonExportExcel" /><asp:PostBackTrigger ControlID="lnkCohortAI" /></Triggers>
</asp:UpdatePanel>
<asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="udpCohort"><ProgressTemplate><div class="modal"><div class="center"><asp:Image ID="imgProgress" runat="server" ImageAlign="AbsMiddle" ImageUrl="~/Controls/Images/WaitImage2.gif" />Please Wait...</div></div></ProgressTemplate></asp:UpdateProgress>
