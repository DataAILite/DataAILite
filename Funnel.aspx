<%@ Page Language="VB" AutoEventWireup="false" CodeFile="Funnel.aspx.vb" Inherits="Funnel" %>
<!DOCTYPE html>
...
<asp:UpdatePanel ID="udpFunnel" runat="server">
<ContentTemplate>
<table>
...
&nbsp;&nbsp;&nbsp;&nbsp;<asp:HyperLink ID="HyperLinkHelp" runat="server" NavigateUrl="DataAIHelp.aspx?hilt=Funnel" Target="_blank" CssClass="NodeStyle" Font-Names="Arial">Help</asp:HyperLink>
...
<br /><br />
<div style="text-align:center;"><asp:Label ID="lblHeader" runat="server" Font-Size="22px" Font-Names="Arial">Funnel Analysis</asp:Label><asp:Label ID="LabelAnalysisSubtitle" runat="server" CssClass="analysisSubtitle" Text="Analyze ordered stages or statuses, showing counts, drop-off, conversion, and value at each step."></asp:Label></div>
<span class="analysisRecommendation">Work Flow suggested: review the complete results in <a class="NodeStyle" href="MarketChurn.aspx">Market Churn</a> and <a class="NodeStyle" href="MarketSegments.aspx">Market Segments</a>.</span>
<br />
<table class="controlpanel" cellpadding="4" cellspacing="0">
<tr><td style="font-weight:bold;">Stage Field:</td><td><asp:DropDownList ID="DropDownPrimaryField" runat="server" Width="190px" ToolTip="Status or process step field used as the funnel stage."></asp:DropDownList></td><td style="font-weight:bold;">Value Field:</td><td><asp:DropDownList ID="DropDownValueField" runat="server" Width="190px" ToolTip="Optional numeric field summarized at each funnel stage."></asp:DropDownList></td></tr>
<tr><td style="font-weight:bold;">Stage order:</td><td colspan="3"><asp:TextBox ID="txtStageOrder" runat="server" Width="560px" ToolTip="Optional comma-separated stage order. Leave blank to use the stage values found in the data."></asp:TextBox></td></tr>
<tr><td style="font-weight:bold;">Search:</td><td colspan="3"><asp:TextBox ID="txtSearch" runat="server" Width="420px" ToolTip="Optional text used to filter rows shown in the analysis grid."></asp:TextBox><asp:Button ID="ButtonBuild" runat="server" CssClass="ticketbutton" Text="Build" ToolTip="Build or refresh the page results using the selected controls. The Build action also saves an Excel snapshot of the results in the session temporary folder so it can be included later in Export Packages if needed." /><asp:Button ID="ButtonReset" runat="server" CssClass="ticketbutton" Text="Reset" ToolTip="Clear search and rebuild the default analysis." /><asp:Button ID="ButtonExportCSV" runat="server" CssClass="ticketbutton" Text="CSV" ToolTip="Export the analysis grid to CSV." /><asp:Button ID="ButtonExportExcel" runat="server" CssClass="ticketbutton" Text="Excel" ToolTip="Export the analysis grid to Excel." /><asp:LinkButton OnClientClick="return showWaitingPanel();" ID="lnkFunnelAI" runat="server" CssClass="aiLinkButton" Font-Names="Arial" ToolTip="Ask AI to interpret the analysis grid and method explanation.">AI</asp:LinkButton></td></tr>
</table>
...
<div style="overflow:auto; max-width:100%;"><asp:GridView ID="GridViewFunnel" runat="server" CssClass="analysisgrid" AutoGenerateColumns="True" GridLines="Both"></asp:GridView></div>
...
<Triggers><asp:PostBackTrigger ControlID="ButtonExportCSV" /><asp:PostBackTrigger ControlID="ButtonExportExcel" /><asp:PostBackTrigger ControlID="lnkFunnelAI" /></Triggers>
</asp:UpdatePanel>
<asp:UpdateProgress ID="UpdateProgress1" runat="server" AssociatedUpdatePanelID="udpFunnel"><ProgressTemplate><div class="modal"><div class="center"><asp:Image ID="imgProgress" runat="server" ImageAlign="AbsMiddle" ImageUrl="~/Controls/Images/WaitImage2.gif" />Please Wait...</div></div></ProgressTemplate></asp:UpdateProgress>
